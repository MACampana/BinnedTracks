...
#Create DAGMAN file
dag_path="${scratch_dir}/job_files/dags/BinnedTrials_${where_acc}_dagman.dag"
touch ${dag_path}
...
for s in ${seeds[@]}; do
    ...
    exec_path="${scratch_dir}/job_files/execs/BinnedTrials_${where_acc}_${s}_exec.sh"
    touch ${exec_path}
    ...
    sub_path="${scratch_dir}/job_files/subs/BinnedTrials_${where_acc}_${s}_submit.submit"
    touch ${sub_path}
    echo "executable = ${exec_path}" >> ${sub_path}
    echo "output = ${scratch_dir}/outputs/BinnedTrials_${where_acc}_${s}.out" >> ${sub_path}
    echo "error = ${scratch_dir}/errors/BinnedTrials_${where_acc}_${s}.err" >> ${sub_path}
    echo "log = ${scratch_dir}/logs/BinnedTrials_${where_acc}_${s}.log" >> ${sub_path}        
    ...
    echo "JOB BinnedTrials_${where_acc}_${s} ${sub_path}" >> ${dag_path}
    
done

#Below is the Submit file. After running this script, run the below shell file to submit the jobs.
runThis="${scratch_dir}/job_files/SubmitMyJobs_BinnedTrials_${where_acc}.sh"
touch ${runThis}
echo "#!/bin/sh" >> ${runThis}
