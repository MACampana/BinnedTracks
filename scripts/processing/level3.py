...
def i3f2npy(path, assign_time, fix_leap=True, MC=False):#, nfiles=None, neventsperfile=None):
    ...
    for frame in i3f:
        ...
        if 'SplineMPE' not in frame.keys():
            continue
        
        if 'SplineMPEMuEXDifferential' not in frame.keys():
            continue     
            
        if 'SRTHVInIcePulses_Qtot' not in frame.keys():
            continue 
            
        if 'MPEFitParaboloidFitParams' not in frame.keys():
            continue      
        ...
        zen.append(frame['SplineMPE'].dir.zenith)
        azi.append(frame['SplineMPE'].dir.azimuth)
        ...
        qtot.append(frame['SRTHVInIcePulses_Qtot'].value)
        qtot_wdc.append(frame['SRTHVInIcePulses_QtotWithDC'].value)
        
        angErr.append(np.sqrt(frame['MPEFitParaboloidFitParams'].pbfErr1**2 + frame['MPEFitParaboloidFitParams'].pbfErr2**2) / np.sqrt(2) )
...
def hdf2npy(path, assign_time, fix_leap=True, MC=False):#, nfiles=None, neventsperfile=None):
    ...
    zen = hdf['SplineMPE']['zenith']
    azi = hdf['SplineMPE']['azimuth']
    if assign_time is None:
    ...
    logE = np.log10(hdf['SplineMPEMuEXDifferential']['energy'])
    qtot = hdf['SRTHVInIcePulses_Qtot']['value']
    qtot_wdc = hdf['SRTHVInIcePulses_QtotWithDC']['value']

    angErr = np.sqrt(frame['MPEFitParaboloidFitParams']['err1']**2 + frame['MPEFitParaboloidFitParams']['err2']**2) / np.sqrt(2)
