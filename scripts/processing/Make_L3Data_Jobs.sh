...
#Create DAGMAN file
dag_path="${scratch_dir}/job_files/dags/Process_L3Data_dagman.dag"
touch ${dag_path}
...
for y in ${years[@]}; do
    for d in ${dates[@]}; do
        ...
        filelist2=(/data/ana/Muon/level3/exp/$y/$d/Run*/Level3_IC86*.i3.zst)
        
        #Create executable job file
        exec_path="${scratch_dir}/job_files/execs/Process_L3Data_${d}${y}_exec.sh"
        touch ${exec_path}
        ...
        echo "output = ${scratch_dir}/outputs/Process_L3Data_${d}${y}.out" >> ${sub_path}
        echo "error = ${scratch_dir}/errors/Process_L3Data_${d}${y}.err" >> ${sub_path}
        echo "log = ${scratch_dir}/logs/Process_L3Data_${d}${y}.log" >> ${sub_path}        
        ...
        echo "JOB Process_L3Data_${d}${y} ${sub_path}" >> ${dag_path}

    done
done

#This is the Submit file. After running Make_Cluster_Jobs.sh, run the below shell file to submit the jobs.
runThis="${scratch_dir}/job_files/SubmitMyJobs_Process_L3Data.sh"
touch ${runThis}
echo "#!/bin/sh" >> ${runThis}
