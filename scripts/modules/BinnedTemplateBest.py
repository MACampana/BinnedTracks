...
class BinnedTemplateAnalysis:
    """
    For conducting binned, maximum-likelihood, template analyses of IceCube event data.
    ...
    def __init__(self, data, sig, grl, template, is_binned=False, savedir=None, name='BinnedTemplateAnalysis', 
        ...
        self.sig_relweights = self.sig_evs['ow'] * self.sig_evs['trueE']**(-self.gamma)
        ...
        for e in range(self.n_logE_bins):
            ...
            #Get weighted median
            med_sigs[e] = self.weighted_quantile(self.sig_evs['trueAngErr'][e_mask], self.sig_relweights[e_mask], 0.5)
    ...
    def bin_data(self, data, verbose=None, sig=False, weights=None):
        ...
        if sig:
            event_decs = data['trueDec']
            event_ras = data['trueRa']
        else:
            event_decs = data['dec']
            event_ras = data['ra']
    ...
    def create_data_acc_dist(self, skw={}):
        """
        Create detector acceptance distribution (data/*background*, sinDec-dependent).
        ...
        self.binned_bg_hists = bg_hist 
        #print('bg done')
        return
    
    def create_signal_acc_dist(self, skw={}):
        """
        Create detector acceptance distribution (*signal*, sinDec-dependent).
        ...
        for e in range(self.n_logE_bins):
            ...
            h_counts = hl.hist(np.sin(self.sig_evs['trueDec'][sig_ebin_mask]), weights=self.sig_relweights[sig_ebin_mask], bins=np.sin(bin_edges)) 
            #Normalize such that integral over solid angle = 1
            h = h_counts.normalize(density=True) / (2*np.pi)
            h = h.gaussian_filter1d(sigma=2) 
            #Fit spline
    ...
    def get_acc_from_dist(self, sindec, e, acc):
        """
        Used distribution to get acceptance at a give sin(Dec) for a given energy bin index.
        ...
        Args:
            ...
            acc: One of "signal" or "bg" for which acceptance distribution to use.
        ...
        if acc == 'signal':
            try:
                out = self.sig_hists[e].get_values(sindec) 
                
            #If the signal_hists has not been created...
            except AttributeError:
                print('Signal acceptance distribution not yet created. Creating now... \n')
                self.create_signal_acc_dist()
                out = self.sig_hists[e].get_values(sindec) 
        ...
        #Same for background...
            ...
            try:
                out = self.binned_bg_hists[e].get_values(sindec) 
                
            except AttributeError:
                print('Background acceptance distribution not yet created. Creating now... \n')
                self.create_data_acc_dist()
                out = self.binned_bg_hists[e].get_values(sindec)             
                
            return out
        
        else: #if acc is not signal or bg
            raise ValueError('Argument acc must be one of ["signal", "bg"].')
    ...
    def create_template_pdf(self, smooth_sigs):
        ...
        if not hasattr(self, 'sig_hists'):
            #Make acceptance distribution, if not already made
            self.create_signal_acc_dist()
        ...
        for e in range(self.n_logE_bins):
            ...
            template_acc = template * self.get_acc_from_dist(np.sin(self.bin_decs), e, acc='signal')                   
    ...
    def multinomial_TS(self, n_sig, n, p_s, p_b, p_ssub, frac=1.0):
        ...
        if self.sigsub:
            ...
            TS = 2.0 * np.sum( n * np.log( np.maximum( ( (frac*n_sig) / (N * p_b) ) * (p_s - p_ssub) + 1.0 , 1e-12 ) ) ) #Maximum ensures the log term is > 0 for few outlying cases where it is not already
        else:
    ...
    def get_pdfs_sigsub(self, verbose=None):
        ...
        p_b = np.array([self.get_acc_from_dist(np.sin(self.bin_decs), b, acc='bg') for b in range(self.n_logE_bins)])   
    ...
    def scrambler(self, seed=None, verbose=None):
        """
        Gets a map of "scrambled" counts by sampling from Poisson at given sindec
        ...
        for e in range(self.n_logE_bins):
            ...
            for i,dec in enumerate(unique_decs):
                ...
                val = self.get_acc_from_dist(np.sin(dec), e, acc='bg') * num_bg_events * hp.nside2pixarea(self.nside) 
    ...
    def template_injector(self, n_sig, seed=None, verbose=None, poisson=True):
        ...
        for e in range(self.n_logE_bins):
            ...
            for i in np.unique(bin_sindec_bin_inds):
                ...
                sig_dec_mask = np.digitize(np.sin(sig['trueDec']), np.sin(bin_edges)) == i
            ...
            true_ev_coord = ap.coordinates.SkyCoord(inj_evs['trueRa'], inj_evs['trueDec'], unit='rad', frame='icrs')
