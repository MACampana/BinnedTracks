...
class LiMaStats:
    ...
    def create_template_pdf(self, smooth_sig):
        ...
        if self.where_acc in ['both', 'temp']:
            self.template_pdf = self.template_acc_smoothed.copy()
        elif self.where_acc == 'comb':
            self.template_pdf = self.template_smoothed.copy()
    ...
    def allsky_llr(self, acc='signal', verbose=None):
        ...
        *Note*: Data may not have been re-binned after injections! 
        ...
        if self.where_acc in ['both', 'comb']:
            allsky_llr = np.sum(accs * llrs)
        elif self.where_acc == 'temp':
            allsky_llr = np.sum(llrs)
    ...
    def create_bin_count_spline(self):
        ...
        count_bins = np.linspace(0, np.quantile(lima_bean.binned_data, 0.99), 100)
        h = hl.hist((lima_bean.binned_data, np.sin(lima_bean.bin_decs)), 
